...
const levers = [
  {
    ...
    tradeoffs: [
      { label: "Too small", effect: "More precise match, but lose context. Model may answer from fragments." },
      { label: "Too large", effect: "More complete info, but harder to retrieve precisely. Blows context window." },
      { label: "Sweet spot", effect: "200-800 tokens with 10-20% overlap. Use semantic splits at sentence/heading boundaries." },
    ],
  },
  {
    icon: Brain,
    ...
    tradeoffs: [
      { label: "Higher dims", effect: "More expressive but heavier compute. 384 → 768 → 1536 dimensions." },
      { label: "Domain-specific", effect: "Code, legal, medical may benefit from specialized embeddings." },
      { label: "Latency/cost", effect: "Batch embedding for ingestion; query side is single-shot — speed matters." },
    ],
  },
  {
    icon: Layers,
    ...
    tradeoffs: [
      { label: "Exact (brute-force)", effect: "Great for small corpora (<10k docs). Very simple infra." },
      { label: "ANN (approximate)", effect: "FAISS, HNSW, ScaNN. Needed for millions+ chunks." },
      { label: "Hybrid search", effect: "BM25 lexical + vector. Handles keywords AND semantic queries." },
    ],
  },
  {
    icon: Target,
  ...
  },
  {
    ...
    tradeoffs: [
      { label: "Strict grounding", effect: "\"Answer using ONLY the context. If not present, say you don't know.\"" },
      { label: "Citation request", effect: "Ask for source chunk IDs or document titles for transparency." },
      { label: "Structured output", effect: "Request JSON for downstream processing. Step-by-step reasoning." },
    ],
  },
];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            The knobs you can turn — think of these as <span className="font-semibold">trade-offs</span>, not magic
          </p>
        </motion.div>
